...
public class DiffUtil {
    ...
    public static String diff(Object oldObj, Object newObj) throws NoSuchMethodException, InvocationTargetException,
        ...
        for (java.lang.reflect.Field field : oldFields) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("filedName:" + filedName + " filedClass:" + filedClass + " method:"
                        + oldCurrentMethod.getName() + "valueClass:"
                        + (oldValue == null ? null : oldValue.getClass().getName()) + "oldValue:" + oldValue
                        + " newValue:" + newValue);
            }
    ...
    private static Method getMethodByName(Method[] oldMethods, String filedName) {
        String  methodNameSuffix = filedName.substring(0, 1).toUpperCase() + filedName.substring(1);
        for (Method method : oldMethods) {
            String methodName = method.getName();
            boolean isMatch = methodName.equals("get" + methodNameSuffix) ||methodName.equals(filedName) || methodName.equals("is" + methodNameSuffix);
            if (isMatch) {
                return method;
            }
        }
